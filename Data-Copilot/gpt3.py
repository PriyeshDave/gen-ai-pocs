...
def generate_sql_gpt(_data_schema, new_question, _sample_data, _schema_data2, _sample_data2):
    if _schema_data2 is None and _sample_data2 is None:
        ...
        Answer: 
        <sql_start>
        SELECT COUNT(DISTINCT employee_id) as employee_count, laptop_model
        FROM DATA INNER JOIN DATA2
        ON DATA.employee_id DATA2.employee_id
        WHERE laptop_model = 'macbook air'
        <sql_end>
        ...
        Answer: 
        <sql_start>
        SELECT COUNT(DISTINCT employee_id) as employee_count, laptop_model, year_of_joining
        FROM DATA INNER JOIN DATA2
        ON DATA.employee_id DATA2.employee_id
        WHERE laptop_model = 'macbook air' AND
